...
const renderDetailWithRangeDefaults = renderDetail;
renderDetail = (symbol, range, interval) => {
  const currentRange = [...document.querySelectorAll(".chart-actions [data-range]")].find((button) => button.classList.contains("active"))?.dataset.range;
  return renderDetailWithRangeDefaults(symbol, range, currentRange === range ? interval : undefined);
};
