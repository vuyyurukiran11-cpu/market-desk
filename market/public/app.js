...
function dashboardTemplate() {
  return `<section class="page"><div class="page-head"><div><div class="eyebrow">Personal research workspace</div><h1>Your market desk</h1></div><div class="status" id="freshness">Loading prototype market data…</div></div><section class="panel-grid"><article class="panel"><h2>ETFs</h2><div class="quote-list" id="etfs"></div></article><article class="panel"><h2>Watchlist</h2><div class="quote-list" id="watchlist"></div></article><article class="panel"><h2>Today's top stocks</h2><p>Ranking rules are intentionally deferred until a supported market screener is selected.</p><span class="warning">Coming later</span></article><article class="panel"><h2>Day trading stocks</h2><p>Scanner criteria and data requirements have not been set for v1.</p><span class="warning">Coming later</span></article><article class="panel"><h2>Metals</h2><div class="quote-list" id="metals"></div></article></section><section class="holdings"><div class="holdings-head"><h2>My holdings</h2><div class="holding-filters"><input id="holding-filter" type="search" placeholder="Filter by ticker" aria-label="Filter holdings by ticker"><select id="holding-status" aria-label="Filter holdings"><option value="all">All positions</option><option value="profit">Profitable</option><option value="loss">Losses</option></select></div></div><div class="table-wrap"><table><thead><tr><th class="remove-column" aria-hidden="true"></th><th><button class="sort-button" data-sort="symbol">Ticker <span></span></button></th><th><button class="sort-button" data-sort="quantity">Qty <span></span></button></th><th>Purchase price</th><th>Current price</th><th>Total value</th><th><button class="sort-button" data-sort="profit">Unrealized P/L <span></span></button></th><th>Status</th></tr></thead><tbody id="holdings-body"></tbody></table></div></section></section>`;
}
...
async function loadDashboardData() {
  ...
  try {
    ...
    document.querySelector("#holding-filter").oninput = () => renderHoldings(holdings, bySymbol);
...
function renderHoldings(holdings, bySymbol) {
  ...
  const ticker = document.querySelector("#holding-filter").value.trim().toUpperCase();
  const rows = holdings.filter((holding) => holding.symbol.toUpperCase().includes(ticker)).map((holding) => { const quote = bySymbol.get(holding.symbol), profit = quote?.price != null && holding.purchasePrice != null ? (quote.price - holding.purchasePrice) * holding.quantity : null, totalValue = quote?.price != null ? quote.price * holding.quantity : null; return { holding, quote, profit, totalValue }; }).filter(({ profit }) => status === "all" || (status === "profit" ? profit > 0 : profit < 0)).sort((a, b) => { const left = a[holdingSort.key] ?? a.holding[holdingSort.key], right = b[holdingSort.key] ?? b.holding[holdingSort.key]; return typeof left === "string" ? left.localeCompare(right) * holdingSort.direction : ((left ?? -Infinity) - (right ?? -Infinity)) * holdingSort.direction; });
